...
def generate_tweet(graph, min_distance=5, max_distance=8, start_word=None):
    ...
    while i > 0:
        ...
        choices = list(graph[current_word].keys())
        weights = list(map(lambda x: x[0], graph[current_word].values()))
        next_word = np.random.choice(choices, None, p=weights)

        # Check if the chosen word is a stop word (dead end)
        if graph[current_word][next_word][1] == True:
        ...
        if len(tweet_words) >= min_distance:
            end_probability = graph[current_word][next_word][2] * end_try / num_end_tries
            end_try += 1
...
def generate_new_tweets(number_tweets, markov_graph, start_words):
    ...
    errors = client.insert_rows_json(table_id, tweets)
    if errors == []:
...
def create_new_tweets(event, context):
    markov_graph = load_pickle(bucket, folder, 'markov_graph')
    start_words = load_pickle(bucket, folder, 'start_words')
    generate_new_tweets(100, markov_graph, start_words)


create_new_tweets(None, None)